...
// primitives the protocol needs (issue #108), namespaced per cluster identity
...
type Store interface {
	...
	// before the cluster forms frees it and a later arrival can re-win (issue
	// #108, decision 1). Once any node has registered, the roster is non-empty
	// and callers join instead of claiming, so the claim is never re-won under a
	// live cluster.
	...
	// (keepalive-as-re-register, issue #108 decision 7), so a dead member's
	// entry expires and is pruned from the roster.
